...
```{r Setup data frame}
  ...
  # Name vectors
    colnames(measures.state) <- c("Bundesland", "Startdatum", ">=1000",
                                  ">=100", ">=50", ">=10", ">=5",
                                  "Schulschließung", "Geschäftsschließung",
                                  "Reiserückkehrer", "Lockdown")
```
...
```{r Fill data frame with information}
  ...
  # Baden-Württemberg
  measures.state <- measures.state %>% rbind(.,
    c("Baden-Württemberg", "2020-03-18",	1, 1,	1, 1, 1, 1, 1, 1, 0),
    c("Baden-Württemberg", "2020-03-11", 1, 0, 0, 0, 0, 0, 0, 0, 0)
  )
  # Bayern
  measures.state <- measures.state %>% rbind(.,
  ...
  # Thüringen
  measures.state <- measures.state %>% rbind(.,
    c("Thüringen", "2020-03-11", 1, 0, 0, 0, 0, 0, 0, 0, 0),
    c("Thüringen", "2020-03-17", 1, 1, 1, 0, 0, 1, 0, 0, 0),
    c("Thüringen", "2020-03-19", 1, 1, 1, 1, 0, 1, 1, 1, 0)
  )

#setwd("C:/Users/Leoni/Google Drive/WirvsVirus")
#save(measures.state, file = "measures_state.RData")
```
